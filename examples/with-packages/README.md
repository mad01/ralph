...
Packages are managed with `ralph update`. Remote packages are cloned into `packages_dir` (default `~/.config/ralph/pkg/`), then built and installed using the specified commands. Local packages skip the clone step and run build/install from their `working_dir`.

- `ralph update` -- update and rebuild all packages.
- `ralph update fzf` -- update and rebuild only fzf.
...
3. Run `ralph update` to clone, build, and install packages.
