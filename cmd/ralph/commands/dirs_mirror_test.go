...
func TestApplyDirsMirror_SymlinkDir(t *testing.T) {
	...
	_ = os.MkdirAll(filepath.Join(absSrc, "skill-a"), 0o755)
	_ = os.MkdirAll(filepath.Join(absSrc, "skill-b"), 0o755)
	// Create a hidden dir that should be skipped
	_ = os.MkdirAll(filepath.Join(absSrc, ".hidden"), 0o755)
...
func TestApplyDirsMirror_SymlinkFiles(t *testing.T) {
	...
	relSrc := "rc"
	absSrc := filepath.Join(repoDir, relSrc)
	_ = os.MkdirAll(absSrc, 0o755)
	_ = os.WriteFile(filepath.Join(absSrc, "01-env.zsh"), []byte("# env"), 0o644)
	_ = os.WriteFile(filepath.Join(absSrc, "02-aliases.zsh"), []byte("# aliases"), 0o644)
	_ = os.WriteFile(filepath.Join(absSrc, ".hidden"), []byte("# hidden"), 0o644)
...
func TestApplyDirsMirror_SkipsDisabled(t *testing.T) {
	...
	relSrc := "src"
	_ = os.MkdirAll(filepath.Join(repoDir, relSrc), 0o755)
	_ = os.WriteFile(filepath.Join(repoDir, relSrc, "file.txt"), []byte("x"), 0o644)
...
func TestApplyDirsMirror_DryRun(t *testing.T) {
	...
	absSrc := filepath.Join(repoDir, relSrc)
	_ = os.MkdirAll(absSrc, 0o755)
	_ = os.WriteFile(filepath.Join(absSrc, "file.txt"), []byte("x"), 0o644)
...
func TestBuildIntendedManifest_DirsMirrorSymlinkAction(t *testing.T) {
	...
	absSrc := filepath.Join(repoDir, relSrc)
	_ = os.MkdirAll(absSrc, 0o755)
	_ = os.WriteFile(filepath.Join(absSrc, "file.zsh"), []byte("x"), 0o644)
